...
def Loss_design(out, feature_map, x):
    ...
    loss_data_h = loss_data(out[mask_have][...,:5], feature_map[mask_have][...,:5])
    loss_data_n = loss_data(out[mask_none][...,:5], feature_map[mask_none][...,:5])

    loss_class=nn.MSELoss()
    Softmax=nn.Softmax()
    loss_class_h = loss_class(Softmax(out[mask_have][..., 5:]), feature_map[mask_have][..., 5:])
    loss_class_n= loss_class(out[mask_none][..., 5:], feature_map[mask_none][..., 5:])
    Loss = loss_data_h+loss_class_h + (1 - x) * loss_data_n+(1 - x) * loss_class_n
...
def Supervision(img_data,out_13,out_26,out_52):
    ...
    dict_reverse = {0: '狗', 1: '人', 2: '羊驼', 3: '小汽车', 4: '自行车', 5: '海豚', 6: '松鼠', 7: '马', 8: '猫'}
    ...
    for box in save_boxes:
        ...
        img_draw.rectangle((int(box[1] - box[3] / 2), int(box[2] - box[4] / 2), int(box[1] + box[3] / 2), int(box[2] + box[4] / 2)),outline="red",width=2)
        img_draw.text((int(box[1] - box[3] / 2), int(box[2] - box[4] / 2)), text=dict_reverse[int(box[5])],fill=(0, 255, 255), font=Font)
    plt.imshow(img_back)
    plt.pause(0.1)
    plt.ioff()
...
if __name__ == '__main__':
    ...
    labels_path = r"F:\Datasets_Dispose\Yolo_Datasets\Labels_Yolo-1.txt"
    net_path = r"F:\jkl\Yolo\Net_Save\Yolo_net.pth"
